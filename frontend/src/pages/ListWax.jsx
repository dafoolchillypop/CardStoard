...
function SortWaxModal({ sortConfig, defaultSort, onApply, onClose }) {
  const [levels, setLevels] = useState([...sortConfig]);
  const [setAsDefault, setSetAsDefault] = useState(false);
  const usedKeys = new Set(levels.map(l => l.key));
  const available = WAX_SORT_COLUMNS.filter(c => !usedKeys.has(c.key));
      ...
      <div className="modal-box" style={{ width: 640, maxWidth: "95vw" }} onClick={e => e.stopPropagation()} onKeyDown={e => e.stopPropagation()}>
        ...
        {levels.map((level, i) => (
          <div key={i} style={{ display: "flex", gap: "0.5rem", alignItems: "center", marginBottom: "0.5rem" }}>
            ...
            <select value={level.key} onChange={e => updateLevel(i, "key", e.target.value)} style={{ ...selStyle, flex: 1 }}>
              {WAX_SORT_COLUMNS.filter(c => c.key === level.key || !usedKeys.has(c.key)).map(c => (
        ...
        <div style={{ display: "flex", gap: "0.75rem", marginTop: "1rem", justifyContent: "center" }}>
          ...
          <button className="nav-btn" onClick={async () => {
            if (setAsDefault) {
              try { await api.put("/settings/", { default_sort_wax: levels }); } catch (e) {}
            }
            onApply(levels);
            onClose();
          }}>Apply</button>
...
export default function ListWax() {
  ...
  useEffect(() => {
    Promise.all([api.get("/wax/"), api.get("/settings/")])
      .then(([itemsRes, settingsRes]) => {
        setItems(itemsRes.data);
        setSettings(settingsRes.data);
        setPinnedId(settingsRes.data.pinned_wax_id || null);
      })
      .catch(err => console.error("Error loading wax boxes:", err))
  ...
  useEffect(() => {
    ...
    if (settings.default_sort_wax?.length > 0) {
      setSortConfig(settings.default_sort_wax);
    }
  }, [settings]);
  ...
  const handlePinRow = async (id) => {
    ...
    setPinnedId(next);
    try { await api.put("/settings/", { pinned_wax_id: next }); } catch (e) {}
  };
  ...
  const handleEditSave = async (item) => {
    ...
    try {
      const res = await api.patch(`/wax/${item.id}`, payload);
  ...
  const handleDelete = async (item) => {
    const label = `${item.year} ${item.brand}${item.set_name ? " " + item.set_name : ""}`;
    if (!window.confirm(`Delete "${label}"?`)) return;
    try {
      await api.delete(`/wax/${item.id}`);
      setItems(prev => prev.filter(b => b.id !== item.id));
      if (pinnedId === item.id) {
        setPinnedId(null);
        try { await api.put("/settings/", { pinned_wax_id: null }); } catch (e) {}
      }
    } catch (err) {
  ...
  const handleDuplicate = async (item) => {
    ...
    try {
      const res = await api.post("/wax/", payload);
      setItems(prev => {
  ...
  const handleAddNew = () => {
    const blank = { id: "new", year: "", brand: "", set_name: "", quantity: "1", value: "", notes: "" };
    setEditingId("new");
    setEditForm({ ...blank });
  ...
  const handleRefreshValue = async (item) => {
    try {
      const res = await api.post(`/wax/${item.id}/refresh-value`);
      setItems(prev => prev.map(b => b.id === item.id ? res.data : b));
    } catch (err) {
  ...
  const handlePrintLabel = async (item) => {
    try {
      const res = await api.get(`/wax/${item.id}/public`);
    ...
    <>
      ...
      <div className="list-container">
        ...
        <div style={{ display: "flex", alignItems: "center", padding: "0 1rem", marginBottom: "0.5rem", gap: "0.5rem", flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", fontSize: "0.9rem", color: "var(--text-secondary)" }}>
            ...
            {hasFilters && (
              <><span>&middot;</span>
                <span role="button" tabIndex={0} onClick={() => { setYearFilter(""); setBrandFilter(""); setTypeFilter("all"); setOpenFilterCols(new Set()); }} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setYearFilter(""); setBrandFilter(""); setTypeFilter("all"); setOpenFilterCols(new Set()); } }}
                ...
                <tr>
                  ...
                  <th style={{ width: 70, textAlign: "center", padding: "4px 6px" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "2px" }}>
                      <span role="button" tabIndex={0} style={{ cursor: "pointer", userSelect: "none" }} onClick={() => requestSort("year")} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); requestSort("year"); } }}>Year{getSortIndicator("year")}</span>
                      <span role="button" tabIndex={0} style={{ cursor: "pointer", fontSize: "0.75rem", opacity: 0.6 }} onClick={() => toggleFilter("year")} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); toggleFilter("year"); } }} title="Filter by year">🔍</span>
                    </div>
                  ...
                  <th style={{ width: 100, textAlign: "center", padding: "4px 6px" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "2px" }}>
                      <span role="button" tabIndex={0} style={{ cursor: "pointer", userSelect: "none" }} onClick={() => requestSort("set_name")} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); requestSort("set_name"); } }}>Type{getSortIndicator("set_name")}</span>
                      <span role="button" tabIndex={0} style={{ cursor: "pointer", fontSize: "0.75rem", opacity: 0.6 }} onClick={() => toggleFilter("set_name")} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); toggleFilter("set_name"); } }} title="Filter by type">🔍</span>
                    </div>
                  ...
                  {/* Qty */}
                  <th style={{ width: 60, textAlign: "center", padding: "4px 6px" }}>
                    <span role="button" tabIndex={0} style={{ cursor: "pointer", userSelect: "none" }} onClick={() => requestSort("quantity")} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); requestSort("quantity"); } }}>Qty{getSortIndicator("quantity")}</span>
                  </th>
                  ...
                  {/* Total */}
                  <th style={{ width: 90, textAlign: "center", padding: "4px 6px" }}>Total</th>
                ...
                {items.length === 0 && editingId === null && (
                  <tr>
                    <td colSpan={8} style={{ textAlign: "center", padding: "2rem", color: "var(--text-muted)" }}>
                      No wax boxes yet. Click ＋ to add one.
                    </td>
                  </tr>
                )}
                {filtered.map(item => {
                      ...
                      <td className="action-col actions-col" style={{ textAlign: "center" }}>
                        {isEditing ? (
                          ...
                          <div style={{ display: "flex", gap: "0.4rem", justifyContent: "center", flexWrap: "wrap" }}>
                            ...
                            <button onClick={() => window.open(`/wax-view/${item.id}`, "_blank")}
                              style={{ background: "none", border: "none", cursor: "pointer", fontSize: "1.3rem", padding: "2px 4px", color: "#6c757d", width: "auto" }}
      ...
      {showSortModal && (
        <SortWaxModal
          ...
          defaultSort={settings?.default_sort_wax || []}
      ...
      {labelData && (
        <LabelPreviewModal
          labelData={labelData}
          onPrint={() => { window.open(`/wax-label/${labelData.id}`, "_blank"); setLabelData(null); }}
